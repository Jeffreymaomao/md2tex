...
function md2ast(markdown, config={}) {
    ...
    const regex_unorderedList_dash = /^(?:>\s*| )*- /;
    const regex_unorderedList_star = /^(?:>\s*| )*\* /;
    const regex_orderedList        = /^(?:>\s*| )*\d+\. /;
    const regex_displayCode        = /^(?:>\s*| )*`{3}(\w*)$/;
    const regex_displayMath        = /^(?:>\s*| )*\${2}/;
    const regex_horizontalRule     = /^(?:>\s*| )*-{3}/;
    const regex_table              = /^(?:>\s*| )*\|(.+)\|$/;
    const regex_tableSeparator     = /^(?:>\s*| )*\|(\s*:?-+:?\s*\|)+\s*$/;
    const regex_tableAlignment     = /(?:>\s*| )*\|?\s*(:)?-+(:)?\s*/g;
    ...
    lines.forEach((line, index) => {
        ...
        // --- START : displayCode ---
        if (regex_displayCode.test(line)) {
            // 如果父層的深度存在的話
            ...
            if(!isDisplayCode) {
                ...
                if(previousChild && previousChild.type === "paragraph"){
        ...
        // --- START : list ---
            ...
            let matchTabLength = 2;
            if((match = regex_unorderedList_dash.exec(line))!=null){
                if(match) matchTabLength = match[0].length;
            } else if((match = regex_unorderedList_star.exec(line))!=null){
                if(match) matchTabLength = match[0].length;
            }else if((match = regex_orderedList.exec(line))!=null){
                if(match) matchTabLength = match[0].length;
            }
            ...
            const item = {
                type: 'listItem',
                depth: indentDepth,
                children: [...parseInlineContent(line.substring(matchTabLength).trim(), regex_inline)]
            };
            // ---
